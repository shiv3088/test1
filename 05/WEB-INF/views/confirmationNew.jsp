...
<link href="/resources/flight-result/css/angular-busy.css" rel="stylesheet">
 <link href="/resources/flight-result/css/result-style.css?34234234234" rel="stylesheet">
...
<header>
  <nav class="navbar navbar-default navbar-static-top">
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-sm-4 col-xs-5"> <a class="navbar-brand" href="/"><img src="/resources/flight-result/images/logo.png?v1" alt="Travelopick Flight Booking"></a> </div>
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-sm-12">
				...
				<p><span class="hidden-xs">Need help, Our </span> 24x7 Toll Free Support : <a href="tel:${data.tfnNo}">${data.tfnNo}</a></p>
			...
			<div class="hidden-xs">
				         ...
				         <c:choose>
							<c:when test="${booking.macp =='Yes'}">
								...
								<a href="tel:${data.tfnNo}" onClick="ga('send', 'event', 'Phone Call', 'Click to Call', '${data.tfnNo}');" style="width:100%;">
...
<%-- 					<p>Your booking has been successfully received, You may also buzz at<br/><b>+44-800-972-3035</b> (Toll Free). Your booking reference number is <strong><c:out value='${booking.bookingId}' /></strong></p> --%>
					<p>Your booking has been successfully received, You may also buzz at<br/><b>${data.tfnNo}</b> (Toll Free). Your booking reference number is <strong><c:out value='${booking.bookingId}' /></strong></p>                        
				 </c:when>
				 ...
				 <c:choose>
				  ...
				  <c:when test="${booking.pnr=='X'}">				
					...
					<p>Our Associate will reach you shortly. You may also buzz at<br/>
						<!-- <b>+44-800-972-3035</b> --> <b>${data.tfnNo}</b> (Toll Free). Your booking reference number is </p>
				...
				<c:if test="${booking.flexSearch ==  'yes' || booking.flexSearchTo ==  'yes'}">
					         ...
					         <p class="atten-txt">
<!-- 					         	<b>Kind Attention:</b> This is a flexible date that you Booked. In case you would like to enquire more about this you may call us on our toll free number: +44-800-972-3035.  -->
					         	<b>Kind Attention:</b> This is a flexible date that you Booked. In case you would like to enquire more about this you may call us on our toll free number: ${data.tfnNo}. 
					         </p>          
					         ...
					         <c:if test="${booking.nearBy ==  'yes' || booking.nearByTo ==  'yes'}">
					          ...
					          <p class="atten-txt">
<!-- 					         	<b>Kind Attention:</b> This is a Near By Airport that you Booked. In case you would like to enquire more about this you may call us on our toll free number: +44-800-972-3035.  -->
					         	<b>Kind Attention:</b> This is a Near By Airport that you Booked. In case you would like to enquire more about this you may call us on our toll free number: ${data.tfnNo}. 
					         </p>          
		...
		<c:set var="flightIti" value="${flightIti} ${flight.originAirport.airportCode} - ${flight.destinationAirport.airportCode}" scope="page" />						  
				   ...
				   <div class="return itineraryblock">
					...
					<div class="row margin_top">
						...
						<div class="col-xs-4">
							...
							<div class="time"><fmt:formatDate type="date" dateStyle="full" value="${dateObject }" pattern="dd/MM/yyyy"/> | <fmt:formatDate value="${dateObject }" pattern="HH:mm" /></div>
                ...
                <c:otherwise>
						...
						<div class="col-xs-4 text-right">
						   ...
						   <fmt:parseDate value="${dateParts}" var="dateObject" pattern="yyyy-MM-dd HH:mm:ss" />
							<div class="time"><fmt:formatDate type="date" dateStyle="full" value="${dateObject }" pattern="dd/MM/yyyy"/> | <fmt:formatDate value="${dateObject }" pattern="HH:mm"  />
							 </div>
            ...
            <table>
				...
				<div class="paymentBox">
					...
					<div class="gray_bg"><strong>Price Details (${booking.displayCurrencyCode})</strong></div>
					
					<ul>
						
						<c:if test="${booking.pageSource > 0}">
						<c:if test="${booking.fare.noOfAdult > 0}">
						 <fmt:formatNumber var="cAdultFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${(booking.fare.adultFare) * (booking.currencyConvertion)}" />
						 <fmt:formatNumber var="pubAdultFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${((booking.fare.adultFare) * (booking.currencyConvertion))*booking.fare.noOfAdult}" />
						...
						<c:if test="${booking.fare.noOfChild > 0}">
						 	<fmt:formatNumber var="cChildFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${(booking.fare.childFare) * (booking.currencyConvertion)}" />
						 	<fmt:formatNumber var="pubChildFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${((booking.fare.childFare) * (booking.currencyConvertion))*booking.fare.noOfChild}" />
						...
						<c:if test="${booking.fare.noOfInfant > 0}">
						    <fmt:formatNumber var="cInfantFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${(booking.fare.infantFare) * (booking.currencyConvertion)}" />
						 	<fmt:formatNumber var="pubInfantFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${((booking.fare.infantFare) * (booking.currencyConvertion))*booking.fare.noOfInfant}" />
						...
						<c:if test="${booking.fare.noOfInfantWs > 0}">
							<fmt:formatNumber var="cInfantWsFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${(booking.fare.infantWsFare) * (booking.currencyConvertion)}" />
						 	<fmt:formatNumber var="pubInfantWsFare" type="number" minFractionDigits="2" maxFractionDigits="2" value="${((booking.fare.infantWsFare) * (booking.currencyConvertion))*booking.fare.noOfInfantWs}" />
						...
						<fmt:formatNumber var="tax" type="number" minFractionDigits="2" maxFractionDigits="2" value="${(booking.fare.tax+booking.finalSeatPrice+booking.fare.agentFees+((booking.fare.markup+booking.fare.instantDiscountAmt)*(booking.fare.noOfAdult+booking.fare.noOfChild+booking.fare.noOfInfant+booking.fare.noOfInfantWs))) * booking.currencyConvertion}" />
						<c:if test="${tax != 0.00}">
				...
				<div class="add_horizontal">
					...
					<p><small>*All Fares displayed are quoted in ${booking.displayCurrencyCode} and inclusive of base fare, taxes and all fees.<c:if test="${booking.flight.gds != 9}"> Additional 
					<a class="show-desktop" onClick="window.open('/baggage-fees-info?airline=${booking.bagAirline}', 'info', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=900,height=600, screenX=50,screenY=50')" href="javascript: void(0);"> Baggage Fees </a>
			        ...
			        <c:choose>  
	                    <c:when test='${booking.displayCurrencyCode   != "GBP" }'>
					          <div class="pricetotal" style="border:1px solid #dbd4a7">
                                <div class="col-xs-6">
                                    <b>Total charge equivalent in GBP:</b>
                                </div>
                        ...
                        <div style="color:#056b2e; padding:5px 10px 10px 0px;; margin-bottom:-10px; font-size:12px;">
                           ...
                           <b>Please Note:</b> Your Credit/Debit Cards will be charged in GBP for a total amount of <i class="fa fa-dollar"/></i><c:out value='${publishedFare}'/>.
