		...
		<div class="matrix-slider row-flex">
                   ...
                   <div class="matrix-list-data" ng-repeat="item in Newmatrix | startFrom:currentPage*pageSize | limitTo:pageSize">
                     ...
                     <div class="filter-logo" ng-if="item.multi != '0'" ng-click="airlineShow(item.airlineName+'M')" ng-switch on="item.airlineName">
                       ...
                       <div class="non-stop one-stop" ng-class="{active : matrixActive === (item.nonStopPrice+item.airlineName+'0') || (matrixActive === item.nonStopPrice+item.airlineName+'M'+'0') || (matrixActive === item.airlineName+'M' && item.multi != '0' && item.nonStopPrice > 0) || (matrixActive === item.airlineName && item.multi == '0' && item.nonStopPrice > 0)}" ng-switch on="item.nonStopPrice" ng-if="item.legs != 0">
                       ...
                       <div class="one-stop" ng-class="{active : matrixActive === (item.oneStopPrice+item.airlineName+'1') || (matrixActive === item.oneStopPrice+item.airlineName+'M'+'1') || (matrixActive === item.airlineName+'M' && item.multi != '0' && item.oneStopPrice > 0) || (matrixActive === item.airlineName && item.multi == '0' && item.oneStopPrice > 0)}" ng-switch on="item.oneStopPrice" ng-if="item.slegs != 0">
                       <span ng-switch-when="0">
      					-
    					</span>
    					<span ng-switch-default>
                    ...
                    <div class="non-stop one-stop" ng-class="{active : matrixActive === (item.twoStopPrice+item.airlineName+'2') || (matrixActive === item.twoStopPrice+item.airlineName+'M'+'2') || (matrixActive === item.airlineName+'M' && item.multi != '0' && item.twoStopPrice > 0) || (matrixActive === item.airlineName && item.multi == '0' && item.twoStopPrice > 0)}" ng-switch on="item.twoStopPrice" ng-if="item.tlegs != 0" id="twoStopIdCol{{$index}}">
