...
<link href="/resources/flight-result/css/jquery-ui.min.css?28092017" rel="stylesheet">
<link href="/resources/flight-result/css/result-style.css?11102019" rel="stylesheet">
...
<script src="/resources/flight-result/js/activeTracker.js?03112017"></script>
<!--global js impliment  -->
...
<script src="/resources/flight-result/js/listing.js?451859"></script>
<!--on page script-->
...
<script src="/resources/flight-result/js/index_eng_min_1.0.js?345345345"></script>
<!--Search-->
<script src="/resources/flight-result/js/jquery.validate.min.js"></script>
<!--Validation -->
<script src="/resources/flight-result/js/theme-scripts.js"></script>
<!--date time convert  -->
        ...
        <form:form method="POST" action="/search" modelAttribute="flightSearch" id="flightSearch">
          ...
          <div class="holder">
            ...
            <div class="row">
              ...
              <div class="col-lg-6 no-padding">
                <div class="col col-md-3 col-xs-6 input" id="departDateDiv" style="cursor: pointer;">
                  <form:hidden path="froDate" id="departDate" class="form-control"  readonly="true" data-date-autoclose="true" data-date-role="start" name="tbDepart" placeholder="yyyy/mm/dd" />
                  <span class="calender-icon fa fa-calendar"></span>
				  <span class="date-text" id="departDateText">05-Jul-2019</span>
				  </div>
                <div class="col col-md-3 col-xs-6 input" id="returnDateLabel" style="cursor: pointer;">
                  <form:hidden path="toDate" id="returnDate"  class="form-control"  readonly="true" data-date-role="end" name="tbReturn" placeholder="yyyy/mm/dd" />
                  <span class="calender-icon fa fa-calendar"></span> 
				  <span class="date-text" id="returnDateText">05-Jul-2019</span>
				  </div>
...
<p>If you would like for your infant to occupy a reserved seat, the infant must be booked as a CHILD and the applicable children's fare will apply.</p>
        ...
        <div id="resultsCntr" >
          ...
          <div class="row">
            <div class="col-sm-12">
                ...
                <c:forEach items="${searchListWrapper}" var="search" varStatus="mb">
                    ...
                    <fmt:parseDate value="${returns[0]}" var="rdateObject" pattern="MM-dd-yyyy" />
                    <p class="title">${search.originAirport.cityName} to ${search.destinationAirport.cityName} &nbsp;  <small><fmt:formatDate value="${ddateObject }" pattern="EEE, dd MMM, yyyy" /> | <fmt:formatDate value="${rdateObject }" pattern="EEE, dd MMM, yyyy" /></small></p>
                </c:forEach>
                <p class="note-msg" ng-show="list.length"><b>Note:</b> *All Fares displayed are quoted in {{dollar | uppercase}}  and inclusive of base fare, taxes and all fees. Additional <a target="_blank" href="/baggage-fees" style="color:#4863db;">baggage fees</a> may apply as per the airline policies. Some flights displayed either may be for nearby airport(s).</p>
            </div>
          ...
          <!--  Sorting Div start--> 
            ...
            <div class="sorting-tab results-tabs-left pull-left" ng-show="list.length">
			 ...
			 <div class="tab-btn-wrap"> <a id="nbr" order="duration_shorter" ng-show="nearBylist.length" ng-click="clearFilter();nearBySearch()" class="tab-btn lowest-price" href="#near-airport"  aria-controls="near-airport">Near By Airport <span style="color:#4863db; font-size:12px;">(<i class="fa fa-{{dollar}}" ></i>{{nearBylist[0].PersonFare * conversionRatio | number:2}})</span></a></div>
              <!-- <div class="tab-btn-wrap"> <a id="slR" order="price_low_to_high" ng-click="clearFilter();mainList()"  class="tab-btn lowest-price tabactive" href="#src-result"  aria-controls="src-result"> All Result</a> <a id="nbr" order="duration_shorter" ng-show="nearBylist.length" ng-click="clearFilter();nearBySearch()" class="tab-btn shortest-time only-mobile" href="#near-airport"  aria-controls="near-airport">Near By Airport <span class="hidden-xs"  style="color:#4863db; font-size:12px;">(<i class="fa fa-{{dollar}}" ></i>{{nearBylist[0].PersonFare * conversionRatio | number:2}})</span></a> <a id="alr" order="flexible_dates" ng-show="flexlist.length" ng-click="clearFilter();changeList()" class="tab-btn flex-dates" href="#2days"  aria-controls="2days">Flexible Dates <span  class="" style="color:#e91d26; font-size:12px;">(<i class="fa fa-{{dollar}}" ></i>{{flexlist[0].PersonFare * conversionRatio | number:2}})</span></a> </div> -->
            </div>
          </div>
              ...
              <div class="result">
                <div class="hidden-xs">
                  ...
                  <div class="col-bg">
                    ...
                    <div class="col-sm-5">
                      <!--    <div class="flight_score_block" ng-if="Flights.RouteScore >= 0">
                            {{Flights.RouteScoreWord}} {{Flights.routeScoringAmenity.SpeedText}}
                        <div class="flight-score">{{Flights.RouteScore}}</div>
    
                        <div class="flight-score-icon"><img src="/resources/flight-result/images/route_happy/{{happy_img(Flights.RouteScore)}}.png" alt="Route Happy"></div>
                    ...
                    <div class="row" ng-repeat="Segments in Flights.OutMobSegments">
                        ...
                        <div class="col-md-5 col-sm-5 col-xs-6">
                            <div class="vt-trip">
                                ...
                                <div class="triptime">
                                    <ul>
                                        <li><span title="{{Segments.departureTime | date: 'HH:mm'}} {{Segments.departureTime | date: 'EEE,dd MMM'}}">{{Segments.departureTime | date: 'HH:mm' | lowercase}} </span> <i ng-if="Flights.FlexFare == true && $index == 0" title="This is a flexible date" ></i></li>
                                        <li ng-if="!Segments.layoverAirport"><img src="/resources/flight-result/images/loc-dis-icon.png"></li>
                                        <li ng-if="Segments.layoverAirport.split(',').length == 1 && Segments.layoverAirport"><img src="/resources/flight-result/images/loc-dis-icon-1.png"></li>
                                        <li ng-if="Segments.layoverAirport.split(',').length == 2"><img src="/resources/flight-result/images/loc-dis-icon-2.png"></li>
                                        <li ng-if="Segments.layoverAirport.split(',').length == 3"><img src="/resources/flight-result/images/loc-dis-icon-3.png"></li>
                                        <li ng-init="day_count = differenceInDays(Segments.departureTime,Segments.arrivalTime)">{{Segments.arrivalTime | date: 'HH:mm'| lowercase}}<sup ng-if="day_count > 0">+{{day_count}}</sup></li>
                                    </ul>
                                </div>
                                <div class="tripdate">
                                    <ul>
                                ...
                                <ul>
                                        ...
                                        <li>
                                          <span ng-if="!Segments.layoverAirport" class="span">&nbsp;</span>
                                          <span ng-if="Segments.layoverAirport" ng-repeat="layAir in Segments.layoverAirport.split(',')" ng-class="{span_1:Segments.layoverAirport.split(',').length == 1,span_2:Segments.layoverAirport.split(',').length == 2,span_3:Segments.layoverAirport.split(',').length == 3}">{{ layAir }}<!-- <br />56m --></span>
                                        </li>
         ...
         <div class="flightdetails-container">
			<ul>
				...
				<li><span ng-if="Flights.Airline != 'CCF' && Flights.Airline != 'DL'" ng-show="OpenPopupWindowcase(Flights.OutSegments)"><i class="fa fa-suitcase" aria-hidden="true"></i> 
                      ...
                      <div class="row">
                        ...
                        <div class="col-sm-5 col-xs-12">
                          <div class="row">
                            <div class="col-xs-7">
                              <div class="city">
                                <div class="time"> <span title="{{Segments.departureTime | date: 'HH:mm'}} {{Segments.departureTime | date: 'EEE,dd MMM'}}">{{Segments.departureTime | date: 'HH:mm ' | lowercase}} <span class="date" style="color:{{flexColor(Flights.FlexFare,$index)}}" >{{Segments.departureTime | date: 'EEE,dd MMM'}}</span></span> <i ng-if="Flights.FlexFare == true && $index == 0" title="This is a flexible date" class="fa fa-calendar text-red"></i> </div>
                                <div class="time" title="{{Segments.arrivalTime | date: 'HH:mm'}} {{Segments.arrivalTime | date: 'EEE,dd MMM'}}"> {{Segments.arrivalTime | date: 'HH:mm '| lowercase}} <span class="date">{{Segments.arrivalTime | date: 'EEE,dd MMM'}}</span> </div>
                              </div>
                            </div>
                            <div class="col-xs-5">
                              <div class="city1">
                        ...
                        <div class="col-sm-3 col-xs-6">
                          <div class="class text-right">
                            <div class="class-stop"> <strong> <span ng-if="Segments.techStops == '0'">Non Stop</span> <span ng-if="Segments.techStops == null">Non Stop</span> <span ng-if="Segments.techStops == '1\n'">Non Stop</span> <span class="one-stop-air" ng-if="Segments.techStops == '1'"> <b>One Stop</b> <span class="one-stop-air-hover popover left" ng-repeat="tech in Segments.techStopDetails"><i class="icon icon-arrow arrow"></i> <strong>Stops In</strong> <span> {{tech.OriginAirport}}-{{getAirportName(tech.OriginAirport)}}</span><br>
                              <small>Arr {{tech.ArrivalTime | date: 'HH:mm'}} </small> - <small>Dep {{tech.DepartureTime | date: 'HH:mm'}} </small>
                              ...
                              <div class="basic-space-top"  ng-if="Segments.baggage.indexOf('0P') > -1"><i class="fa fa-suitcase " aria-hidden="true"></i> <span class="baggage-info"><a href="javascript:void(0);">Check-in Baggage: 0P</a> <span class="baggage_detail"><i class="arrow"></i> Hand Luggage Only</span> </span></div>
                      ...
                      <div class="row">
                        ...
                        <div class="col-sm-5 col-xs-12">
                          <div class="row">
                            <div class="col-xs-7">
                              <div class="city">
                                <div class="time"><span title="{{Segments.departureTime | date: 'HH:mm'}} {{Segments.departureTime | date: 'EEE,dd MMM'}}">{{Segments.departureTime | date: 'HH:mm'| lowercase}} <span class="date" style="color:{{flexColor(Flights.FlexFareTo,$index)}}">{{Segments.departureTime | date: 'EEE,dd MMM'}}</span></span> <i ng-if="Flights.FlexFareTo == true && $index == 0" title="This is a flexible date" class="fa fa-calendar text-red"></i> </div>
                                <div class="time" title="{{Segments.arrivalTime | date: 'HH:mm'}} {{Segments.arrivalTime | date: 'EEE,dd MMM'}}">{{Segments.arrivalTime | date: 'HH:mm'| lowercase}} <span class="date">{{Segments.arrivalTime | date: 'EEE,dd MMM'}}</span></div>
                              </div>
                            </div>
                            <div class="col-xs-5">
                  ...
                  <p>Your fare is a combination of two one-way tickets,each subject to its own rules and restrictions.Any change or cancelation in any flight would not alter the other one.Changes to the other flight may incur a charge.</p>
                        ...
                        <div class="col-xs-5">
                          <div class="mobile-price text-right">
                            <div  ng-if="Flights.Airline != 'CCF'"> <span class="partial-price total-fare" ng-if="Flights.Airline != 'AS'" ><i class="fa fa-{{dollar}}" ></i>{{Flights.PersonFare * conversionRatio | number:2}}</span> <span class="partial-price total-fare" ng-if="Flights.Airline == 'AS'"><i class="fa fa-{{dollar}}" ></i>{{(Flights.PersonFare)* conversionRatio | number:2}}</span> <i class="fa fa-chevron-right mobile_price_arrow" aria-hidden="true"></i></div>
                            <a class="select-flight-btn" ng-if="Flights.Airline == 'CCF'" href="tel:<c:out value='${tollFree}' />"><i class="fa fa-phone" aria-hidden="true"></i> Call </a> </div>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-xs-4 text-left"> <i class="fa fa-long-arrow-right pull-right time-top" aria-hidden="true"></i>
                          <div class="city">
                            <div class="city-name"><strong>{{Segments.originAirport}}</strong> <span>{{Segments.departureTime | date: 'HH:mm' | lowercase}}</span> </div>
                            <div class="time"> <span class="date">{{Segments.departureTime | date: 'EEE,dd MMM'}}</span> </div>
                          </div>
                        </div>
                        <div class="col-xs-4 text-left">
                          <div class="city1">
                            <div class="city-name"><strong>{{Segments.destinationAirport}}</strong> <span>{{Segments.arrivalTime | date: 'HH:mm' | lowercase}}</span></div>
                          </div>
                        </div>
      ...
      <div id="leftCntr" class="myClass" ng-show="list.length">
        ...
        <div class="holder">
          ...
          <div class="tab-content">
            ...
            <div id="tab-1" class="filter-block tab-pane active">
       ...
       <span>  Mixed Airline</span></label></li>
                ...
                <ul class="checkbox" >
                  <li ng-repeat="layover in layOverAirport | limitTo: layoverlimit" id="airlines-filter"><small class="farerate"></small>
                    ...
                    <span>{{layover}}</span> </label>
                      ...
                      <div align="right" ng-hide="layOverAirport.length<3" style="cursor:pointer; color:#4863db; padding:15px; text-align: center; border:1px solid #4863db; clear:both;" > <a ng-hide="layoverlimit === layOverAirport.length" style="color:#4863db;" ng-click="layoverlimit = layoverlimit +(layOverAirport.length-3)">Show More <i class="fa fa-angle-down" aria-hidden="true"></i></a> <a ng-show="layoverlimit === layOverAirport.length && layOverAirport.length>3" class="text-red" ng-click="layoverlimit = 3">Show less <i class="fa fa-angle-up" aria-hidden="true"></i></a> </div>
			...
			<div class="filter-block tab-pane abc">
              <div class="head">Why Book with Travelopick?</div>
               
              <div class="content">
			  <div class="checkbox-sec" id="price-filter">
                &#10003; &nbsp; Best Price Guarantee <br /> 
				&#10003; &nbsp; Customer Support round the clock<br />
				 &#10003; &nbsp; Quick, Easy and Safe booking flow<br /><br />
				 <div align="center">
 ...
 <!--    <div class="bottom-call-bar"> <a href="tel:+44-800-972-3035" onClick="ga('send', 'event', 'Phone Call', 'Click to Call', '+44-800-972-3035');"> Nothing Is Free | But For A Deal <i class="fa fa-phone"></i></a> </div> -->
    <div class="bottom-call-bar"> <a href="tel:${data.tfnNo}" onClick="ga('send', 'event', 'Phone Call', 'Click to Call', '${data.tfnNo}');"> Nothing Is Free | But For A Deal <i class="fa fa-phone"></i></a> </div>
  </div>
</div>
</div>
<!-- Footer Start -->
...
<loading>
  <c:forEach items="${searchListWrapper}" var="search" varStatus="mb">
    <div class="result-mid-sec">
        <img  style="margin-top: 30px;" src="/resources/travelopick/images/logo.png?v1" />
     <div style="width: 100%; float: left; clear: both;"><img src="/resources/images/loadingresult.gif?v1" width="140"   /></div>
      <div class="location">
        <c:out value='${search.originAirport.cityCode}'/>
        <span>
        <c:choose>
          <c:when test="${search.tripType=='2'}"><img src="/resources/flight-result/images/round-arrow.png?30062017"> </c:when>
          <c:otherwise><img src="/resources/flight-result/images/mid-arrow.png?30062017"></i></c:otherwise>
        </c:choose>
        </span>
        <c:out value='${search.destinationAirport.cityCode}' />
      </div>
...
<div align="center" id="divSessionOut" style="display:none; " class="popup-product">
  ...
  <div class="session-exp">
    ...
    <div align="center" valign="middle"><a href="#" onClick="submitForm();" class="btn btn-primary btn-lg searchbtn">Search Again</a> &nbsp; &nbsp;  OR &nbsp; &nbsp; <a href="/" class="btn btn-primary btn-lg searchbtn">Go to Home</a></div>
  </div>
</div>
...
<script>
    ...
    $(".tp-widget-review .readmore").click(function () {
    $(".wrapper-reviews .tp-widget-review p").slideUp('slow');
    $('.tp-widget-review .readmore').removeClass('active');
    $('.tp-widget-review .readmore').show();
    if (($(this).prev(".wrapper-reviews .tp-widget-review p").css("display")) == "none") {
        $(this).prev(".wrapper-reviews .tp-widget-review p").slideDown('slow');
