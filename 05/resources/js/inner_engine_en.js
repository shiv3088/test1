	...
	numberOfMonths:numberOfMonthValue,
            ...
            if (dateDepMin != null && dateDepMin != '') {
                ...
                if (dateRetMin != null) {
                    var rMin = new Date(dateRetMin.getFullYear(), dateRetMin.getMonth(), dateRetMin.getDate());
                    if (dMin > rMin) {
                        $('#sereturnDate').val($.datepicker.formatDate('dd-mm-yy', new Date(dMin)));
                        $("#sereturnDate").focus();
                    }
                    else
                	{
...
myapp.controller('airlineCtrl', ['$http', '$window', '$scope', '$filter', function ($http, $window, $scope, $filter) {
    ...
    $scope.dealClick = function (cabin, trip, f1, f2, f3, f4, t1, t2, t3, t4, depart, ret) {
        ...
        var newDateDep = addDays(new Date(), 10);
        var newDateRet = addDays(new Date(), 20);
        ...
        if (new Date(depart) < newDateDep) {
            departDate= $filter('date')(newDateDep, 'dd-MM-yyyy');
	  console.log("departDate : "+departDate);
        }
        else {      
		console.log("departDate else : "+departDate);  	
            departDate = $filter('date')(new Date(depart), 'dd-MM-yyyy');
        }
        ...
        if (trip == 'RoundTrip') {
            ...
            if(new Date(ret) < newDateRet){
            	document.getElementById('returnDate').value = $filter('date')(newDateRet, 'dd-MM-yyyy');                	
            }else{
            	document.getElementById('returnDate').value = $filter('date')(new Date(ret), 'dd-MM-yyyy');
    ...
    $scope.submitSearch = function (from, to, depart, ret) {
        ...
        {
            var departDate = $filter('date')(new Date(myDate), 'dd-MM-yyyy');
        }
        else 
        {
            var departDate = $filter('date')(new Date(depart), 'dd-MM-yyyy');
        }
	...
	$scope.engineDetails=function(cabin, trip, f1, f2, f3, f4, t1, t2, t3, t4, depart, ret)
	{
		...
		$scope.checkInDateDeal= $filter('date')(new Date(depart), 'dd-MM-yyyy');
		...
		if ($scope.trpTypeDeal == 'OneWay') {
			...
			userdob=$filter('date')(userdob, "dd-MM-yyyy");
		...
		if($scope.trpTypeDeal == 'RoundTrip'){
			...
			if(dept > retu)
				...
				{
				$scope.checkOutDateDeal= $filter('date')(new Date(ret), 'dd-MM-yyyy');
				}
