(function($,W,D)
{
        ...
        setupFormValidation: function()
        {
            ...
            //	console.log($("#payment-card").val());
            	if($("#payment-card").val()=="American Express"){
                       ...
                       $("#ccvimg").attr("src", src);
                       $("#tooltip").html("<a href=\"\" class=\"text_link\">4 Digit Number From Your Card<span></span></a>");
            	}else{
            		   var src = "/resources/images/ccv-img.gif";
                       $("#ccvimg").attr("src", src);
                       $("#tooltip").html("<a href=\"\" class=\"text_link\">3 Digit Number From Your Card<span></span></a>");
            	}
                		  ...
                		  'payment.cardCode': {
                		    ...
                		    messages: {
                		    	'payment.postalCode': { 
                		    	 ...
                		    	 'passengerList[0].Gender': { 
                  					required: "Please provide Gender for traveler",
                  				},
         		   'passengerList[0].FirstName': { 
    ...
    $(D).ready(function($) {
        ...
        jQuery.validator.addMethod("age_val", function(value, element, param) {
        	...
        	if(Age == 'Adult'){
        		html="Booking flights for an unaccompanied minor? Some airlines have restrictions on children under the age of 18 years traveling alone. If you have any<br/> questions, please call us at 1-888 509-5589.";
        		if(param == 0){
        			html="Booking flights for an unaccompanied minor? Some airlines have restrictions on children under the age of 18 years traveling alone. If you have any<br/> questions, please call us at 1-888 509-5589.";
      ...
      /*  $.validator.addMethod(
        ...
        $.validator.addMethod('cccardvalue', function (value) {
         ...
         //   return (value != '0.0');
      	...
      	$.validator.addMethod('titlereq', function (value) {
              return (value != '0');
          }, "Please provide Gender for traveler");
        ...
        $.validator.addMethod('selectcard', function (value) {
         ...
         $.validator.addMethod('selectdate', function (value) {
              return (value != 'Day');
          }, "Please provide Valid Day of Date Of Birth for traveler");
          $.validator.addMethod('selectmonth', function (value) {
              return (value != '0');
          }, "Please select Month of Date Of Birth for traveler");
          $.validator.addMethod('selectyear', function (value) {
              return (value != 'Year');
          }, "Please provide Valid Year of Date Of Birth for traveler ");
...
function setccValue(id) {
	...
	if(id=="American Express"){
	 ...
	 //  $("#Payments_MaskCardNumber").val($("#Payments_CardNumber").val());
	 //  $("#booking").validate().element("#Payments_MaskCardNumber");
...
//	localStorage.setItem('ls.paymentPage',JSON.stringify(bookingdata));
	//ls.lkhistory
	 ...
	 if($('#currentDevice').val()=="Normal"){
		  $('form#booking').attr({action: '/paymentdone/'+rndId});  
     }else if($('#currentDevice').val()=="Mobile"){
    	  $('form#booking').attr({action: '/m/paymentdone/'+rndId});  
     }else if($('#currentDevice').val()=="Tablet"){
    	  $('form#booking').attr({action: '/t/paymentdone/'+rndId});  
     }else{
    	  $('form#booking').attr({action: '/paymentdone/'+rndId});  
     }
...
//	console.log("totalValue:"+totalValue);
	...
	if(types == 'TG'){
		if(status == 'Y'){
			...
			totbsp=(Number(macpValue)+Number(tgInsValue)+Number(totalValue)+Number(brbPrice)+Number(tmValue))-Number(couponAmt);
		//	alert("tgYes:"+totbsp);
		}else{
			 $ttg.slideUp(200);
			 $tgNo.slideDown(200);
		...
		if(status == 'Y'){
			...
			totbsp=(Number(macpValue)+Number(tripMateCost)+Number(totalValue)+Number(brbPrice)+Number(tgValue))-Number(couponAmt);
		//	alert("tgYes:"+totbsp);
		}else{
			 $ttm.slideUp(200);
			 $tgNo.slideDown(200);
		...
		if(status == 'Y'){
			totbsp=(Number(macpValue)+Number(tgValue)+Number(totalValue)+Number(brbPrice)+Number(tmValue))-Number(values);
		}else{
			totbsp=(Number(macpValue)+Number(tgValue)+Number(totalValue)+Number(brbPrice)+Number(tmValue))+Number(0);
		}
	}
	else if(types == 'BRB'){
		if(status == 'Y'){
			brbPrice=brbPrice*totPax;
			totbsp=(Number(macpValue)+Number(tgValue)+Number(totalValue)+Number(brbPrice)+Number(tmValue))-Number(couponAmt);
