...
var month_names =["Jan","Feb","Mar",
	...
	var showToDrop = false;				  
function fromDropdown(){
	
	var fromAirportList = [{'originAirport':'ABZ','originAirportName':'Dyce'}, {'originAirport':'BFS','originAirportName':'Belfast Intl'}, {'originAirport':'BHD','originAirportName':'George Best City Apt'}, {'originAirport':'BHX','originAirportName':'Birmingham'}, {'originAirport':'BOH','originAirportName':'Bournemouth IntL'}, {'originAirport':'BRS','originAirportName':'Bristol'}, {'originAirport':'CWL','originAirportName':'Cardiff'}, {'originAirport':'DSA','originAirportName':'Robin Hood'}, {'originAirport':'EDI','originAirportName':'Edinburgh'}, {'originAirport':'EMA','originAirportName':'East Midlands'}, {'originAirport':'EXT','originAirportName':'Exeter'}, {'originAirport':'GCI','originAirportName':'Guernsey'}, {'originAirport':'GLA','originAirportName':'Glasgow Intl'}, {'originAirport':'HUY','originAirportName':'Humberside'}, {'originAirport':'INV','originAirportName':'Inverness'}, {'originAirport':'IOM','originAirportName':'Ronaldsway'}, {'originAirport':'JER','originAirportName':'Jersey'}, {'originAirport':'LBA','originAirportName':'Leeds Bradford Apt'}, {'originAirport':'LCY','originAirportName':'London City Apt'}, {'originAirport':'LDY','originAirportName':'Eglinton'}, {'originAirport':'LGW','originAirportName':'Gatwick'}, {'originAirport':'LHR','originAirportName':'Heathrow'}, {'originAirport':'LPL','originAirportName':'John Lennon'}, {'originAirport':'LTN','originAirportName':'Luton'}, {'originAirport':'MAN','originAirportName':'Manchester IntL'}, {'originAirport':'MME','originAirportName':'Tees Valley'}, {'originAirport':'NCL','originAirportName':'Newcastle IntL'}, {'originAirport':'NQY','originAirportName':'Cornwall'}, {'originAirport':'NWI','originAirportName':'Norwich'}, {'originAirport':'PIK','originAirportName':'Prestwick'}, {'originAirport':'SEN','originAirportName':'Southend Municipal'}, {'originAirport':'SOU','originAirportName':'Southampton'}, {'originAirport':'STN','originAirportName':'Stansted'}, {'originAirport':'WIC','originAirportName':'Wick'}] ;
 var frominnerHTML="";
	for (var i = 0; i < fromAirportList.length; i++) {
                
frominnerHTML= frominnerHTML+"<li data-name='"+fromAirportList[i].originAirport+"-"+fromAirportList[i].originAirportName+"' class='checkbox'><label><input  type='checkbox' id='fro-"+fromAirportList[i].originAirport+"' value='true'><span>"+fromAirportList[i].originAirport+"-"+fromAirportList[i].originAirportName+"</span></label></li>";
				 
            }
			$("#fromAirportList").html(frominnerHTML);
			
			$('#fromAirportList li').on('click', function() {
			var fromChecked = false;	
			...
			$('#fromAirportList input:checkbox').removeAttr('checked');
			var fromCity = $(this).data("name");
			if(fromChecked){
			...
			$("#froCity").val(fromCity);
			$("#fromAirportDiv").addClass("hide");
			//}
...
function toDropdown(fromCity){
	document.getElementById("toAirportList").innerHTML="";
		  $.getJSON( "/resources/toAirportNew.json", function(response) {
		  frominnerHTML="";
		  var destCountryName = "";
		  $.each(response, function(i, item) {
			if(fromCity == item.originAirport){
				
				if(i == 0){
					frominnerHTML="<div class='autosugguest-head'>"+item.destCountryName+" <i class='fa fa-angle-down'></i></div><ul>";
				}
				else if(item.destCountryName != destCountryName){
					frominnerHTML=frominnerHTML+"</ul><div class='autosugguest-head'>"+item.destCountryName+" <i class='fa fa-angle-down'></i></div><ul>";
				}
				
			frominnerHTML= frominnerHTML+"<li data-name='"+item.destAirport+"-"+item.destAirportName+"' class='checkbox'><input type='checkbox' id='to-"+item.destAirport+"' value='true'><span>"+item.destAirport+"-"+item.destAirportName+"</span></li>";
			destCountryName = item.destCountryName;
			}
		  });
			frominnerHTML = frominnerHTML+"</ul>";
			$("#toAirportList").html(frominnerHTML);
			//$("#toAirportDiv").removeClass("hide");
			showToDrop = true;	
			//$("#toCity").val("");
				$('#toAirportList li').on('click', function() {
				var toChecked = false;	
				if($(this).find('input:checkbox').is(":checked")){
					toChecked = true;
				}
				...
				$('#toAirportList input:checkbox').removeAttr('checked');
				if(toChecked){
					$("#toCity").val("");
				$(this).find('input:checkbox').prop('checked',false);
				}
				else{
				$("#toCity").val($(this).data("name"));
				$(this).find('input:checkbox').prop('checked',true);
				$("#toAirportDiv").addClass("hide");
				}
			});
...
$(document).ready(function() {
	...
	$("#froCity").click(function(){
	$("#fromAirportDiv").removeClass("hide");
	$("#toAirportDiv").addClass("hide");
});
$("#toCity").click(function(){
	if($("#froCity").val() != "" && showToDrop == true){
	$("#toAirportDiv").removeClass("hide");
	$("#fromAirportDiv").addClass("hide");
	}
});	
...
$(".close").click(function(){
	$(".new-autosugguest").addClass("hide");
});
var froAirportCode = $("#froCity").val();
froAirportCode = froAirportCode.substring(froAirportCode.indexOf("(")+1,froAirportCode.indexOf(")"));
$("#fro-"+froAirportCode).prop('checked',true);
toDropdown(froAirportCode);
var toAirportCode = $("#toCity").val();
toAirportCode = toAirportCode.substring(toAirportCode.indexOf("(")+1,toAirportCode.indexOf(")"));
setTimeout(function() {
				$("#to-"+toAirportCode).prop('checked',true);
			},500);


	var dateDep = $("#departDate").datepicker("getDate");
	var dateRet = $("#returnDate").datepicker("getDate");
	$("#departDateText").html(dateDep.getDate()+" "+month_names[dateDep.getMonth()]+" "+dateDep.getFullYear());
	$("#returnDateText").html(dateRet.getDate()+" "+month_names[dateRet.getMonth()]+" "+dateRet.getFullYear());
    ...
    $("#returnToCity").autocomplete({
       ...
       /* if (selectRtTo) {
            $("#returnToCity").val($('ul#ui-id-4 li:first a').text());
            var cityImage = $('ul#ui-id-4 li:first a').text();
            var n = cityImage.indexOf("(");
            var l = cityImage.indexOf(")");
