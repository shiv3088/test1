               ...
               // $scope.searchQueryList.push(localStorageService.get("searchQuery"));
				...
				$scope.OpenPopupWindow = function (lists) {
					...
					// angular.forEach(lists, function(tech) {
				       ...
				       // });
		               $window.open("/us/baggage-fees-info?airline="+$scope.bagairline, "popup", "width=900,height=600,left=10,top=150");
		           }
     ...
     $scope.CabinFilter = function(flights) {
		...
		//	console.log(stops);
            ...
            $scope.reverse = $scope.reverse;
          	...
          	$scope.airlinem = airline;
         	//$scope.filtersPrice.Price = price;
   		    $scope.matrixActive = price + airline + stops;
   		    $scope.stop=stops;
   		    $scope.sortKey = "PersonFare";  
            $scope.reverse = $scope.reverse;
          }
        ...
        $scope.resetReturnSlider= function () {
        	  ...
        	  $slider.slider("values", 1, 1439);
						...
						angular.forEach(data, function(Flighth) {
							...
							try {
								var utmsource=document.getElementById('utmResSource').value;
								console.log("utmsource :- "+utmsource);
								
								if(Flighth.secretDeals != null && (utmsource.toLowerCase() == 'bing' || utmsource.toLowerCase() == 'bingmobile' || utmsource.toLowerCase() == 'google')){
									console.log(Flighth.secretDeals.Price)	
									$scope.secretDealsPrice = Flighth.secretDeals.Price;	
									if($scope.secretDealsPrice > 0){
									var dealTime = "";
											if(localStorage.getItem("underCutPriceSet") == null){
											dealTime = new Date().setMinutes(new Date().getMinutes() + 15);
											localStorage.setItem("underCutPriceSet", dealTime);
											}
											else{
												dealTime = localStorage.getItem("underCutPriceSet") ;
											}
											setTimeout(function(){ 
											if(localStorage.getItem("underCutPriceShow") == null){
											...
											var x = setInterval(function() {
											  ...
											  // Output the result in an element with id="demo"
												$(".Timer").html(minutes + "<small>m</small> " + seconds + "<small>s</small> ");
											  // If the count down is over, write some text 
											  if (distance < 0) {
												clearInterval(x);
												$(".Timer").html("EXPIRED");
												$(".list-count").addClass("hide");
												$(".count-top-icon").addClass("hide");
												localStorage.setItem("underCutPriceShow", false);
											  }
											}, 1000);
											$(".list-count").removeClass("hide");
											}
											 }, 15000); 
											$(".close-btn").click(function(){
												$(".list-count").addClass("hide");
												$(".count-top-icon").removeClass("hide");
												localStorage.setItem("underCutPriceShow", false);
											});
											$(".count-top-icon").click(function(){
												$(".list-count").removeClass("hide");
												$(".count-top-icon").addClass("hide");
											})
										}
			         ...
			         $scope.numberOfPages=function(){
        				return Math.ceil($scope.matrix.length/$scope.pageSize);                
						        ...
						        if($scope.responseStatus.ErrorDescription == "BlackListedCountry"){
									 ...
									 var url = "/us/noResult/NO"+$scope.cityPair;
      								  $window.location.href = url;
						        }
					...
					$scope.shortestFlightList = function (keyname) {
						 $scope.currentPage=0;
							 var myEl = angular.element(document.querySelector('#sft'));
							 ...
							 angular.forEach($scope.list, function(Flighth, index) {
								    ...
								    $scope.$watch($scope.lower_price_slider, function(){
					            	      $scope.lower_price_slider_cur = Math.floor($scope.lower_price_slider * $scope.conversionRatio);
					    	       	      ...
					    	       	      $(".min-price-label").html("<i class='fa fa-"+$scope.dollar+"'></i>"+$scope.lower_price_slider_cur);
					    	       	      $(".max-price-label").html("<i class='fa fa-"+$scope.dollar+"'></i>" +$scope.upper_price_slider_cur);
					    	       	   $scope.resetSlider($scope.lower_price_slider_cur,$scope.upper_price_slider_cur);
							      });
							 $timeout( function(){ $scope.sortKey = keyname;  }, 500);
						}
       ...
       $("#price-range").slider({
			...
			try {
			  ...
			  if($("#directFlight1").is(':checked')){
				  angular.forEach($scope.Newmatrix, function(Flighth, index) {
					 $scope.directMsg = true;
					  if(Flighth.legs != 0){
						  $scope.directMsg = false;
					  }
				  })
			  }
...
$scope.flightsDetails = {};
   ...
   /* $scope.movePaymentPage	 = function(id) {
	...
	$scope.movePaymentPage = function(id) {
     ...
     $window.location.href = '/us/purchase/'+id;
    };
